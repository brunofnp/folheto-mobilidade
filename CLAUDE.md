...
- **Site estático de distribuição (`docs/`) — implementado, release já na produção (2026-09-21).** GitHub Pages está no ar em `https://brunofnp.github.io/folheto-mobilidade/` (Pages configurado no repo pessoal `brunofnp/folheto-mobilidade`, branch `next`, pasta `/docs`). A release oficial dos PDFs (`v1`) está em **`dadosfnp/folheto-mobilidade`** (produção) — `docs/folhetos.json` aponta pra lá. **Nota histórica:** por algumas horas nesta mesma data, `brunofnp` só tinha `pull` em `dadosfnp/folheto-mobilidade` (mesmo tipo de fricção da Decisão 3); resolvido no mesmo dia (admin do org liberou `push`) — nesse meio-tempo a release e o `folhetos.json` chegaram a apontar temporariamente pro repo pessoal, já corrigido. `dadosfnp/folheto-mobilidade` também estava **vazio** até este dia (nunca tinha recebido `git push production main`) — resolvido também. **Ainda pendente:** habilitar GitHub Pages em `dadosfnp/folheto-mobilidade` também (Settings → Pages → branch `main`, pasta `/docs`) se a distribuição final for pelo domínio da organização, não só o pessoal — passo manual, fora do alcance de código.
