...
def cor_por_percentil_invertido(pos: int, total: int):
    """Mesma escala, para métricas onde POSIÇÃO 1 = PIOR (ex.: taxa de
    mortalidade no trânsito — quem mata mais fica em 1º). Espelha
    `cor_por_percentil`: pos=1 vermelho (Q1), pos=total verde (Q5).

    Usar sempre que "1º lugar" for uma notícia ruim — nunca inverter
    manualmente `cor_por_percentil` na chamada, para não repetir esse
    raciocínio em cada tema."""
    if not total:
        return FNP_Q3
    pct = (pos - 1) / total * 100
    if pct <= 20:   return FNP_Q1
